...
class PeppaBattleLevelBase {
    ...
    updateLasers() {
        ...
        const targetWidth = this.gameEnv.innerWidth;
        const targetHeight = this.gameEnv.innerHeight;

        if (this.laserLayer.width !== targetWidth || this.laserLayer.height !== targetHeight) {
            this.laserLayer.width = targetWidth;
            this.laserLayer.height = targetHeight;
            this.laserLayer.style.width = `${targetWidth}px`;
            this.laserLayer.style.height = `${targetHeight}px`;
        }
    ...
    createHud() {
        ...
        this.playerNameEl = document.getElementById(`peppa-player-name-${this.config.levelId}`);
        this.playerHpEl = document.getElementById(`peppa-player-hp-${this.config.levelId}`);
        this.player2HpEl = document.getElementById(`peppa-player2-hp-${this.config.levelId}`);
        this.coinCountEl = document.getElementById(`peppa-coin-count-${this.config.levelId}`);
        this.enemyHpEl = document.getElementById(`peppa-enemy-hp-${this.config.levelId}`);
        this.messageEl = document.getElementById(`peppa-message-${this.config.levelId}`);
    ...
    updateHud(message = null) {
        ...
        if (this.playerNameEl) {
            this.playerNameEl.textContent = `Player: ${this.playerName || 'Player'}`;
        }
        if (this.playerHpEl) {
            this.playerHpEl.textContent = `P1 HP: ${this.playerHealth}/${this.playerMaxHealth}`;
        }
        if (this.player2HpEl && this.gameMode === 'twoPlayer') {
            this.player2HpEl.textContent = `P2 HP: ${this.player2Health}/${this.player2MaxHealth}`;
        }
        if (this.coinCountEl) {
            this.coinCountEl.textContent = `Coins: ${this.coinCount} (${this.coinCount * this.coinValue} bonus)`;
        }
        if (this.enemyHpEl && boss) {
            this.enemyHpEl.textContent = `${this.config.enemyName} HP: ${boss.health}/${boss.maxHealth}`;
        }

        if (this.messageEl && message !== null) {
    ...
    update() {
        ...
        this._hudUpdateFrame = (this._hudUpdateFrame + 1) % 10;
        if (this._hudUpdateFrame === 0) {
            this.updateHud();
        }
    }
}
